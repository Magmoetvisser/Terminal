...
// Top-level tab type
// ============================================================
type TopTab = 'wijzigingen' | 'github';

// ============================================================
// Changes (Wijzigingen) helpers & types
// ============================================================
...
function parseLog(raw: string): LocalCommitInfo | null {
...
function lcStatusColor(s: string): string {
...
function lcStatusIcon(s: string): string {
...
function lcStatusLabel(s: string): string {
...
export default function SourceControlScreen() {
  ...
  // ---- Top-level tab ----
  const [activeTopTab, setActiveTopTab] = useState<TopTab>('wijzigingen');

  // ============================================================
  // Changes (Wijzigingen) state & queries
  // ============================================================
  ...
  const { data: numstatData, refetch: refetchNumstat } = useQuery({
    queryKey: ['git-numstat', projectPath],
    queryFn: () => apiFetch(`/api/git/numstat?path=${encodeURIComponent(projectPath)}`),
    enabled: !!projectPath,
    refetchInterval: 10000,
  });
  ...
  const { data: diffData, isLoading: diffLoading } = useQuery({
    queryKey: ['git-diff', projectPath, expandedFile],
    queryFn: () => apiFetch(`/api/git/diff?path=${encodeURIComponent(projectPath)}&file=${encodeURIComponent(expandedFile!)}`),
    enabled: !!projectPath && !!expandedFile,
  });
  ...
  const fileChanges: LocalFileChange[] = statusFiles.map((f) => {
    const stats = unstagedStats[f.file] || stagedStats[f.file] || { added: 0, deleted: 0 };
    return {
      file: f.file,
      status: f.status,
      statusLabel: lcStatusLabel(f.status),
      added: stats.added,
  ...
  // ============================================================
  // Render: Wijzigingen (Changes) tab content
  // ============================================================
  const renderChangesTab = () => {
    if (!projectPath) {
      return (
        <View style={styles.lc_emptyContainer}>
          <Ionicons name="folder-open-outline" size={48} color={colors.textDim} />
          <Text style={styles.lc_emptyText}>Selecteer eerst een project in de Editor tab</Text>
        </View>
      );
    }
    ...
    return (
      <ScrollView
        style={styles.lc_container}
        refreshControl={<RefreshControl refreshing={false} onRefresh={onRefreshChanges} tintColor={accentColor} />}
      >
        {/* Last commit */}
        {lastCommit && (
          <View style={styles.lc_commitCard}>
            <View style={styles.lc_commitHeader}>
              <Ionicons name="git-commit-outline" size={16} color={accentColor} />
              <Text style={styles.lc_commitLabel}>Laatste commit</Text>
            </View>
            <Text style={styles.lc_commitMessage} numberOfLines={2}>{lastCommit.message}</Text>
            <View style={styles.lc_commitMeta}>
              <Text style={styles.lc_commitHash}>{lastCommit.hash}</Text>
              <Text style={styles.lc_commitDot}>&middot;</Text>
              <Text style={styles.lc_commitAuthor}>{lastCommit.author}</Text>
              <Text style={styles.lc_commitDot}>&middot;</Text>
              <Text style={styles.lc_commitTime}>{lastCommit.timeAgo}</Text>
            </View>
          </View>
        )}

        {/* Commit message input */}
        <View style={styles.lc_commitInputRow}>
          <TextInput
            style={styles.lc_commitInput}
          ...
          />
          <TouchableOpacity
            style={[styles.lc_commitBtn, {
              backgroundColor: commitMessage.trim() ? accentColor : '#333',
            }]}
            onPress={() => {
              if (!commitMessage.trim()) return;
              const escaped = commitMessage.trim().replace(/"/g, '\\"');
              const cmd = projectPath
                ? `cd "${projectPath}" && git add -A && git commit -m "${escaped}"`
                : `git add -A && git commit -m "${escaped}"`;
              runInTerminal(cmd);
              setCommitMessage('');
            }}
            disabled={!commitMessage.trim() || !!gitActionLoading}
          >
            {gitActionLoading === null && !commitMessage.trim() ? (
              <Ionicons name="git-commit-outline" size={18} color="#666" />
            ) : (
              <Ionicons name="checkmark" size={18} color={commitMessage.trim() ? '#0a0a0a' : '#666'} />
            )}
          </TouchableOpacity>
        </View>

        {/* Git action buttons */}
        <Animated.View style={[styles.lc_gitActions, {
          transform: [{
          ...
          <TouchableOpacity
            style={[styles.lc_gitBtn, { backgroundColor: accentColor }]}
            onPress={() => {
              const cmd = projectPath ? `cd "${projectPath}" && git pull` : 'git pull';
          ...
          >
            {gitActionLoading === 'pull' ? (
              ...
              <Ionicons name="arrow-down-outline" size={16} color="#0a0a0a" style={{ marginRight: 4 }} />
            )}
            <Text style={styles.lc_gitBtnText}>Pull</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.lc_gitBtn, { backgroundColor: accentColor }]}
            onPress={() => {
              const cmd = projectPath ? `cd "${projectPath}" && git push` : 'git push';
          ...
          >
            {gitActionLoading === 'push' ? (
              ...
              <Ionicons name="arrow-up-outline" size={16} color="#0a0a0a" style={{ marginRight: 4 }} />
            )}
            <Text style={styles.lc_gitBtnText}>Push</Text>
          </TouchableOpacity>
        </Animated.View>

        {/* Summary bar */}
        {fileChanges.length > 0 && (
          <View style={styles.lc_summaryBar}>
            <Text style={styles.lc_summaryTotal}>{fileChanges.length} bestanden gewijzigd</Text>
            <View style={styles.lc_summaryStats}>
              {totalAdded > 0 && (
                <View style={styles.lc_statBadge}>
                  <Text style={[styles.lc_statText, { color: colors.accent }]}>+{totalAdded}</Text>
                </View>
              )}
              {totalDeleted > 0 && (
                <View style={styles.lc_statBadge}>
                  <Text style={[styles.lc_statText, { color: colors.red }]}>-{totalDeleted}</Text>
                </View>
              )}
            </View>
          </View>
        ...
        {/* Loading */}
        {statusLoading && (
          <View style={styles.lc_loadingContainer}>
            <ActivityIndicator color={accentColor} />
          </View>
        )}

        {/* Clean state */}
        {!statusLoading && fileChanges.length === 0 && projectPath && (
          <View style={styles.lc_cleanCard}>
            <Ionicons name="checkmark-circle" size={40} color={colors.accent} />
            <Text style={styles.lc_cleanText}>Working tree is clean</Text>
            <Text style={styles.lc_cleanSub}>Geen uncommitted wijzigingen</Text>
          </View>
        )}

        {/* File groups */}
        {groups.map((group) => (
          <View key={group.label} style={styles.lc_group}>
            <View style={styles.lc_groupHeader}>
              <Text style={[styles.lc_groupLabel, { color: lcStatusColor(group.items[0].status) }]}>
                {group.label}
              </Text>
              <View style={[styles.lc_groupBadge, { backgroundColor: lcStatusColor(group.items[0].status) + '20' }]}>
                <Text style={[styles.lc_groupCount, { color: lcStatusColor(group.items[0].status) }]}>
                  {group.items.length}
                </Text>
              </View>
            </View>
            ...
            {group.items.map((file) => {
              ...
              return (
                <View key={file.file}>
                  <TouchableOpacity
                    style={[styles.lc_fileRow, isExpanded && { backgroundColor: colors.elevated }]}
                    onPress={() => setExpandedFile(isExpanded ? null : file.file)}
                    activeOpacity={0.7}
                  >
                    <Ionicons
                      name={lcStatusIcon(file.status) as any}
                      size={16}
                      color={lcStatusColor(file.status)}
                    />
                    <Text style={styles.lc_fileName} numberOfLines={1}>
                      {file.file}
                    </Text>
                    <View style={styles.lc_lineStats}>
                      {file.added > 0 && (
                        <Text style={styles.lc_linesAdded}>+{file.added}</Text>
                      )}
                      {file.deleted > 0 && (
                        <Text style={styles.lc_linesDeleted}>-{file.deleted}</Text>
                      )}
                    </View>
                    <Ionicons
                  ...
                  {/* Inline diff */}
                  {isExpanded && (
                    <View style={styles.lc_diffContainer}>
                      {diffLoading ? (
                          ...
                          <View>
                            {diffData.output.split('\n').map((line: string, i: number) => {
                              ...
                              return (
                                <Text
                                  key={i}
                                  style={[styles.lc_diffLine, { backgroundColor: bg, color }]}
                                >
                                  {line}
                                </Text>
                              );
                        ...
                        <Text style={styles.lc_diffEmpty}>Geen diff beschikbaar (nieuw/untracked bestand)</Text>
                      )}
                    </View>
                  )}
                </View>
      ...
      <View style={styles.ghContainer}>
        ...
        {!!selectedRepo && (
          <Animated.View style={[styles.modalContent, { height: MODAL_MAX_HEIGHT, transform: [{ translateY: panelTranslateY }] }]}>
                  ...
                  {/* Tab selector */}
                  <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.ghTabRow} contentContainerStyle={styles.ghTabRowContent}>
                    {TAB_ORDER.map((tab) => (
                      <TouchableOpacity
                        key={tab}
                        style={[styles.ghTab, detailTab === tab && { borderBottomColor: accentColor }]}
                        onPress={() => setDetailTab(tab)}
                      >
                        <Text
                          style={[styles.ghTabText, detailTab === tab && { color: accentColor }]}
                        >
                          {TAB_LABELS[tab]}
                        </Text>
                      </TouchableOpacity>
                              ...
                              {viewingDiff.patch.split('\n').map((line, idx) => {
                                ...
                                return (
                                  <View
                                    ...
                                    style={[
                                      styles.ghDiffLine,
                                      isAdd && styles.ghDiffLineAdd,
                                      isDel && styles.ghDiffLineDel,
                                      isHunk && styles.ghDiffLineHunk,
                                    ]}
                                  >
                                    <Text
                                      style={[
                                        styles.ghDiffLineText,
                                        isAdd && { color: '#4ade80' },
                                        isDel && { color: '#ef4444' },
                                        isHunk && { color: '#a78bfa' },
                                      ]}
                    ...
                    {detailTab === 'prs' && (
                      viewingPR ? (
                        ...
                        <View>
                          ...
                          <ScrollView horizontal nestedScrollEnabled>
                            <View style={styles.ghDiffContainer}>
                              {viewingPRDiff.patch.split('\n').map((line, i) => (
                                <Text
                                  key={i}
                                  style={[
                                    styles.ghDiffLine,
                                    line.startsWith('+') && !line.startsWith('+++') && styles.ghDiffLineAdd,
                                    line.startsWith('-') && !line.startsWith('---') && styles.ghDiffLineDel,
                                    line.startsWith('@@') && styles.ghDiffLineHunk,
                                  ]}
                                >
                                  {line}
                                </Text>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.topTabBar}>
        <TouchableOpacity
          style={[styles.topTab, activeTopTab === 'wijzigingen' && styles.topTabActive]}
          onPress={() => setActiveTopTab('wijzigingen')}
        >
          <Text style={[styles.topTabText, activeTopTab === 'wijzigingen' && { color: accentColor }]}>
            Wijzigingen
          </Text>
          {activeTopTab === 'wijzigingen' && (
            <View style={[styles.topTabIndicator, { backgroundColor: accentColor }]} />
          )}
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.topTab, activeTopTab === 'github' && styles.topTabActive]}
          onPress={() => setActiveTopTab('github')}
        >
          <Text style={[styles.topTabText, activeTopTab === 'github' && { color: accentColor }]}>
            GitHub
          </Text>
          {activeTopTab === 'github' && (
            <View style={[styles.topTabIndicator, { backgroundColor: accentColor }]} />
      ...
      {/* Tab content */}
      {activeTopTab === 'wijzigingen' ? renderChangesTab() : renderGitHubTab()}
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  // ---- Changes (lc_ prefix for conflicts) ----
  lc_container: {
    flex: 1,
  ...
  // ---- GitHub detail tabs (prefixed to avoid conflict with top tabs) ----
  ghTabRow: {
    width: '100%',
    flexGrow: 0,
    flexShrink: 0,
  ...
  // ---- GitHub diff viewer (prefixed to avoid conflict with changes diff) ----
  ghDiffContainer: {
    backgroundColor: '#111',
    borderRadius: 8,
    borderWidth: 1,
